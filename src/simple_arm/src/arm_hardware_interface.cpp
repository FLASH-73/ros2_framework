...
double convert_rad_to_ticks(double rad, const std::string& joint_name, const std::map<std::string, std::pair<double, double>>& calibration) {
    ...
    if (it != calibration.end()) {
        double scale = it->second.first;
        double offset = it->second.second;
        return static_cast<int>(offset + rad * scale);
    }
    return static_cast<int>(2048 + (rad * (2048 / M_PI)));
}
...
hardware_interface::CallbackReturn ArmHardware::on_init(const hardware_interface::HardwareInfo& info) {
    ...
    // Load params (serial_port, baudrate from URDF or node params)
    std::string serial_port;
    try {
        serial_port = info.hardware_parameters.at("serial_port");
    } catch (const std::out_of_range&) {
        serial_port = "/dev/ttyUSB0";  // Default
    }
    uint32_t baudrate = 1000000;
    try {
        baudrate = std::stoi(info.hardware_parameters.at("baudrate"));
    } catch (const std::out_of_range&) {
        // Use default
    }
    ...
    try {
        ...
        // Log error (use rclcpp::get_logger if needed, but in hardware_interface, return error)
        RCLCPP_FATAL(rclcpp::get_logger("ArmHardware"), "Failed to initialize ServoController: %s", e.what());
        return hardware_interface::CallbackReturn::ERROR;
    }

    // Joint mappings and calibration
    ...
    // Parse joint names from info (replace hard-coded list)
    joint_names_.clear();
    ...
    // Validation
    if (hw_commands_.size() != joint_names_.size() || hw_positions_.size() != joint_names_.size() || hw_velocities_.size() != joint_names_.size()) {
        RCLCPP_FATAL(rclcpp::get_logger("ArmHardware"), "Hardware vectors size mismatch.");
        return hardware_interface::CallbackReturn::ERROR;
    }
    for (const auto& joint : info.joints) {
        if (joint.command_interfaces.size() != 1 || joint.command_interfaces[0].name != hardware_interface::HW_IF_POSITION) {
            RCLCPP_FATAL(rclcpp::get_logger("ArmHardware"), "Joint '%s' has invalid command interface.", joint.name.c_str());
            return hardware_interface::CallbackReturn::ERROR;
        }
        if (joint.state_interfaces.size() != 2 || joint.state_interfaces[0].name != hardware_interface::HW_IF_POSITION || joint.state_interfaces[1].name != hardware_interface::HW_IF_VELOCITY) {
            RCLCPP_FATAL(rclcpp::get_logger("ArmHardware"), "Joint '%s' has invalid state interfaces.", joint.name.c_str());
            return hardware_interface::CallbackReturn::ERROR;
        }
    }
...
hardware_interface::CallbackReturn ArmHardware::on_activate(const rclcpp_lifecycle::State&) {
    ...
    std::set<int> all_servo_ids;
    for (const auto& kv : single_servo_joints_) {
        all_servo_ids.insert(kv.second);
    }
    for (const auto& kv : dual_servo_joints_) {
        all_servo_ids.insert(kv.second.first);
        all_servo_ids.insert(kv.second.second);
    }
...
hardware_interface::return_type ArmHardware::read(const rclcpp::Time&, const rclcpp::Duration&) {
    for (size_t i = 0; i < joint_names_.size(); ++i) {
        ...
        if (single_servo_joints_.count(name)) {
            ...
            if (!ticks_opt.has_value()) continue;  // Error, skip
            int16_t ticks = ticks_opt.value();
            position_rad = convert_ticks_to_rad(ticks, name, calibration_);
            ...
            int16_t leader_ticks = leader_ticks_opt.value();
            int16_t follower_ticks = follower_ticks_opt.value();
            double leader_rad = convert_ticks_to_rad(leader_ticks, name, calibration_);
            double follower_rad = convert_ticks_to_rad(4095 - follower_ticks, name, calibration_);  // Mirror follower
            position_rad = (leader_rad + follower_rad) / 2.0;  // Average
        } else if (name == "link6_Slider-8") {
            auto ticks_opt = driver_->get_position(single_servo_joints_.at(name));
            if (!ticks_opt.has_value()) continue;
            int16_t ticks = ticks_opt.value();
            position_rad = convert_ticks_to_gripper_dist(ticks);
        }

        // Unwrapping for continuous joints (e.g., base)
        if (name == "base_link_Revolute-1") {
            auto last_angle_opt = last_wrapped_angles_[name];
            if (last_angle_opt.has_value()) {
                double last_angle = last_angle_opt.value();
                double delta = position_rad - last_angle;
                if (delta > M_PI) {
                    revolutions_[name] -= 1;
                } else if (delta < -M_PI) {
                    revolutions_[name] += 1;
            ...
            unwrapped_positions_[name] = position_rad + revolutions_[name] * 2 * M_PI;
            last_wrapped_angles_[name] = position_rad;
            hw_positions_[i] = unwrapped_positions_[name];
        } else {
            hw_positions_[i] = position_rad;
        }

        // Velocity placeholder (servos don't provide; set to 0 or estimate if possible)
...
hardware_interface::return_type ArmHardware::write(const rclcpp::Time&, const rclcpp::Duration&) {
    ...
    for (size_t i = 0; i < joint_names_.size(); ++i) {
        ...
        // For continuous base: Compute shortest path
        double final_rad = cmd;
        if (name == "base_link_Revolute-1") {
            double current_unwrapped = unwrapped_positions_[name];
        ...
        if (name == "link6_Slider-8") {
            position_ticks = convert_gripper_dist_to_ticks(final_rad);
        } else {
            position_ticks = convert_rad_to_ticks(final_rad, name, calibration_);
        }
